...
s_DBMap* script_get_label_db(void)
{
	return scriptlabel_db;
}
s_DBMap* script_get_userfunc_db(void)
{
	return userfunc_db;
}
 ...
 *------------------------------------------*/
static int buildin_killmonster_sub_strip(struct s_block_list* bl, va_list ap) //same fix but with killmonster instead - stripping events from mobs.
{
...
static int buildin_killmonsterall_sub_strip(struct s_block_list* bl, va_list ap) //Strips the event from the mob if it's killed the old method.
{
...
BUILDIN_FUNC(getmonsterinfo)
{
	...
	case MOB_MODE:
		script_pushint(st, static_cast<int32>(mob->status.mode));
		break;
...
BUILDIN_FUNC(setunitdata)
{
	...
	case BL_MOB:
		...
		case UMOB_MODE:
			md->base_status->mode = (enum e_MonsterMode)value;
			calc_status           = true;
			break;

		case UMOB_AI:
			md->special_state.ai = (e_mob_ai)value;
			break;
