...
struct s_mob_chat* mob_chat(short id)
{
	if (id <= 0 || id > MAX_MOB_CHAT || mob_chat_db[id] == NULL)
		return (struct s_mob_chat*)NULL;
...
static int mob_ai_sub_hard_slavemob(struct s_mob_data* md, unsigned int tick)
{
	...
	if (status_has_mode(&md->status, e_MonsterMode::CANMOVE)) { //If the mob can move, follow around. [Check by Skotlex]
		int old_dist;
...
static bool mob_ai_sub_hard(struct s_mob_data* md, unsigned int tick)
{
	...
	if (tbl->type == BL_ITEM) { //Loot time.
		...
		if (!check_distance_bl(&md->bl, tbl, 0)) { //Still not within loot range.
			if (static_cast<int>(mode & e_MonsterMode::CANMOVE) == 0) { //A looter that can't move? Real smart.
				mob_unlocktarget(md, tick);
				return true;
			}
	...
	//Out of range...
	if (static_cast<int>(mode & e_MonsterMode::CANMOVE) == 0 || (!can_move && DIFF_TICK(tick, md->ud.canmove_tick) > 0)) { //Can't chase. Immobile and trapped mobs should unlock target and use an idle skill.
