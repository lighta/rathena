...
struct s_mob_db *mob_db_data[MAX_MOB_DB+1];
struct s_mob_db *mob_dummy = NULL;	//Dummy mob to be returned when a non-existant one is requested.

struct s_mob_db *mob_db(int mob_id) { if (mob_id < 0 || mob_id > MAX_MOB_DB || mob_db_data[mob_id] == NULL) return mob_dummy; return mob_db_data[mob_id]; }

//Dynamic mob chat database
struct s_mob_chat *mob_chat_db[MAX_MOB_CHAT+1];
struct s_mob_chat *mob_chat(short id) { if(id<=0 || id>MAX_MOB_CHAT || mob_chat_db[id]==NULL) return (struct s_mob_chat*)NULL; return mob_chat_db[id]; }
...
static s_DBMap *mob_item_drop_ratio;

/// Mob skill struct for temporary storage
struct s_internal_mob_skill {
	int16 mob_id; ///< Monster ID. -1 boss types, -2 normal types, -3 all monsters
	struct s_mob_skill skill[MAX_MOBSKILL]; ///< Skills
	uint8 count; ///< Number of skills
};
static s_DBMap *mob_skill_db; /// Monster skill temporary db. s_mob_skill -> mobid

struct s_mob_db *mobdb_exists(uint16 mob_id) {
	struct s_mob_db *db = mob_db(mob_id);
...
static struct s_eri *item_drop_ers; //For loot drops delay structures.
static struct s_eri *item_drop_list_ers;
...
static s_DBMap *mob_summon_db; /// Random Summon DB. struct s_randomsummon_group -> group_id
...
static int mobdb_searchname_array_sub(struct s_mob_db* mob, const char *str)
{
	if (mob == mob_dummy)
		return 1;
...
*------------------------------------------*/
void mob_clean_spotted(s_mob_data *md) {
	int i;
...
*------------------------------------------*/
void mob_add_spotted(s_mob_data *md, uint32 char_id) {
	int i;
...
*------------------------------------------*/
bool mob_is_spotted(s_mob_data *md) {
	int i;
...
int mvptomb_delayspawn(int tid, unsigned int tick, int id, intptr_t data) {
	struct s_npc_data *nd = BL_CAST(BL_NPC, map_id2bl(id));
...
void mob_set_dynamic_viewdata( s_mob_data* md ){
	...
	if( md && !md->vd_changed ){
		...
		s_view_data* vd = (s_view_data*)aMalloc( sizeof( s_view_data ) );

		// Copy the current values
		memcpy( vd, md->vd, sizeof( s_view_data ) );
...
{
	struct s_mob_data *md = (struct s_mob_data*)aCalloc(1, sizeof(struct s_mob_data));
	...
	if(status_has_mode(&md->db->status,e_MonsterMode::LOOTER))
...
int mob_get_random_id(int type, int flag, int lv)
{
	...
	do {
		...
		(flag&0x02 && lv < mob->lv) ||
		(flag&0x04 && status_has_mode(&mob->status,e_MonsterMode::STATUS_IMMUNE) ) ||
		(flag&0x08 && mob->spawn[0].qty < 1) ||
		(flag&0x10 && status_has_mode(&mob->status,e_MonsterMode::IGNOREMELEE|e_MonsterMode::IGNOREMAGIC|e_MonsterMode::IGNORERANGED|e_MonsterMode::IGNOREMISC) )
...
{
	struct s_spawn_data data;

	memset(&data, 0, sizeof(struct s_spawn_data));
	data.m = m;
	data.num = 1;
	data.id = mob_id;
	data.state.size = size;
	data.state.ai =(e_mob_ai) ai;
 ...
 *------------------------------------------*/
int mob_once_spawn(s_map_session_data* sd, int16 m, int16 x, int16 y, const char* mobname, int mob_id, int amount, const char* event, unsigned int size, e_mob_ai ai)
{
	...
	for (count = 0; count < amount; count++)
	{
		...
		{
			struct s_guild_castle* gc = guild_mapindex2gc(map[m].index);
			struct s_guild* g = (gc) ? guild_search(gc->guild_id) : NULL;
			if (gc)
			{
				md->guardian_data = (struct s_guardian_data*)aCalloc(1, sizeof(struct s_guardian_data));
		...
		if (mob_id < 0 && battle_config.dead_branch_active)
			...
			sc_start4(NULL,&md->bl, SC_MODECHANGE, 100, 1, 0, static_cast<int>(e_MonsterMode::AGGRESSIVE|e_MonsterMode::CANATTACK|e_MonsterMode::CANMOVE|e_MonsterMode::ANGRY), 0, 60000);
	}
 ...
 *------------------------------------------*/
int mob_once_spawn_area(s_map_session_data* sd, int16 m, int16 x0, int16 y0, int16 x1, int16 y1, const char* mobname, int mob_id, int amount, const char* event, unsigned int size, e_mob_ai ai)
{
	int i, max, id = 0;
	int lx = -1, ly = -1;
...
{	//Needed because the guild_data may not be available at guardian spawn time.
	struct s_block_list* bl = map_id2bl(id);
	struct s_mob_data* md;
	struct s_guild* g;
	int guardup_lv;
...
int mob_spawn_guardian(const char* mapname, int16 x, int16 y, const char* mobname, int mob_id, const char* event, int guardian, bool has_index)
{
	...
	if( has_index && gc->guardian[guardian].id )
  	{	//Check if guardian already exists, refuse to spawn if so.
		struct s_mob_data *md2 = (TBL_MOB*)map_id2bl(gc->guardian[guardian].id);
 ...
 *------------------------------------------*/
int mob_can_reach(struct s_mob_data *md,struct s_block_list *bl,int range, int state)
{
	int easy = 0;
...
int mob_setdelayspawn(struct s_mob_data *md)
{
	...
	if (status_has_mode(&db->status,e_MonsterMode::STATUS_IMMUNE)) { // Status Immune
	...
	} else if (status_has_mode(&db->status,e_MonsterMode::IGNOREMELEE|e_MonsterMode::IGNOREMAGIC|e_MonsterMode::IGNORERANGED|e_MonsterMode::IGNOREMISC)) { // Plant type
 ...
 *------------------------------------------*/
static int mob_can_changetarget(struct s_mob_data* md, struct s_block_list* target, e_MonsterMode mode)
{
	...
	switch (md->state.skillstate) {
		case MSS_BERSERK:
			if (static_cast<int>(mode&e_MonsterMode::CHANGETARGET_MELEE) == 0)
				return 0;
		...
		case MSS_RUSH:
			return static_cast<int>(mode&e_MonsterMode::CHANGETARGET_CHASE);
		case MSS_FOLLOW:
		case MSS_ANGRY:
		case MSS_IDLE:
		case MSS_WALK:
 ...
 *------------------------------------------*/
static int mob_ai_sub_hard_activesearch(struct s_block_list *bl,va_list ap)
{
	struct s_mob_data *md;
	struct s_block_list **target;
	e_MonsterMode mode;
	...
	nullpo_ret(bl);
	md=va_arg(ap,struct s_mob_data *);
	target= va_arg(ap,struct s_block_list**);
	mode=(e_MonsterMode) va_arg(ap,int);

	//If can't seek yet, not an enemy, or you can't attack it, skip.
	if ((*target) == bl || !status_check_skilluse(&md->bl, bl, 0, 0))
		return 0;

	if (static_cast<int>(mode&e_MonsterMode::TARGETWEAK) != 0 && status_get_lv(bl) >= md->level-5)
		return 0;
	...
	case BL_PC:
		if (((TBL_PC*)bl)->state.gangsterparadise &&
			!status_has_mode(&md->status,e_MonsterMode::STATUS_IMMUNE))
 ...
 *------------------------------------------*/
static int mob_ai_sub_hard_changechase(struct s_block_list *bl,va_list ap)
{
	struct s_mob_data *md;
	struct s_block_list **target;
 ...
 *------------------------------------------*/
static int mob_ai_sub_hard_bg_ally(struct s_block_list *bl,va_list ap) {
	struct s_mob_data *md;
	struct s_block_list **target;
 ...
 *------------------------------------------*/
static int mob_ai_sub_hard_lootsearch(struct s_block_list *bl,va_list ap)
{
	struct s_mob_data* md;
	struct s_block_list **target;
	int dist;
...
static int mob_warpchase_sub(struct s_block_list *bl,va_list ap) {
	struct s_block_list *target;
	struct s_npc_data **target_nd;
	struct s_npc_data *nd;
	int *min_distance;
	int cur_distance;

	target= va_arg(ap, struct s_block_list*);
	target_nd= va_arg(ap, struct s_npc_data**);
 ...
 *------------------------------------------*/
static int mob_ai_sub_hard_slavemob(struct s_mob_data *md,unsigned int tick)
{
	...
	if(status_has_mode(&md->status,e_MonsterMode::CANMOVE))
...
int mob_unlocktarget(struct s_mob_data *md, unsigned int tick)
{
	...
	default:
		...
		if (status_has_mode(&md->status,e_MonsterMode::ANGRY) && !md->state.aggressive)
...
int mob_randomwalk(struct s_mob_data *md,unsigned int tick)
{
	...
	if(DIFF_TICK(md->next_walktime,tick)>0 ||
	   status_has_mode(&md->status,e_MonsterMode::NORANDOM_WALK) ||
	   !unit_can_move(&md->bl) ||
	   !status_has_mode(&md->status,e_MonsterMode::CANMOVE))
		return 0;
 ...
 *------------------------------------------*/
static bool mob_ai_sub_hard(struct s_mob_data *md, unsigned int tick)
{
	struct s_block_list *tbl = NULL, *abl = NULL;
	e_MonsterMode mode;
	...
	can_move = (static_cast<int>(mode&e_MonsterMode::CANMOVE) != 0 && unit_can_move(&md->bl) != 0);
	...
	{	//Check validity of current target. [Skotlex]
		...
		if (!tbl || tbl->m != md->bl.m ||
			...
			(
				tbl->type == BL_PC &&
				((((TBL_PC*)tbl)->state.gangsterparadise && static_cast<int>((mode&e_MonsterMode::STATUS_IMMUNE)) == 0) ||
	...
	if( md->attacked_id && static_cast<int>(mode&e_MonsterMode::CANATTACK) != 0 )
	{
	...
	if (!tbl && can_move && static_cast<int>(mode&e_MonsterMode::LOOTER) != 0 && md->lootitems && DIFF_TICK(tick, md->ud.canact_tick) > 0 &&
	...
	if ((!tbl && static_cast<int>(mode&e_MonsterMode::AGGRESSIVE) != 0) || md->state.skillstate == MSS_FOLLOW)
	{
		map_foreachinallrange (mob_ai_sub_hard_activesearch, &md->bl, view_range, DEFAULT_ENEMY_TYPE(md), md, &tbl, mode);
	}
	else
	if (static_cast<int>(mode&e_MonsterMode::CHANGECHASE) != 0 && (md->state.skillstate == MSS_RUSH || md->state.skillstate == MSS_FOLLOW))
	{
		int search_size;
	...
	if (!tbl) { //No targets available.
		...
		if( md->bg_id && static_cast<int>(mode&e_MonsterMode::CANATTACK) != 0 ) {
			if( md->ud.walktimer != INVALID_TIMER )
	...
	{	//Loot time.
		...
		{	//Still not within loot range.
			if (static_cast<int>(mode&e_MonsterMode::CANMOVE) == 0)
		...
		fitem = (struct s_flooritem_data *)tbl;
	...
	{	//Target within range and able to use normal attack, engage
		...
		{ //Only attack if no more attack delay left
			...
			if( static_cast<int>(mode&e_MonsterMode::RANDOMTARGET) == 0)
	...
	//Out of range...
	if (static_cast<int>(mode&e_MonsterMode::CANMOVE) == 0 || (!can_move && DIFF_TICK(tick, md->ud.canmove_tick) > 0))
...
static int mob_ai_sub_hard_timer(struct s_block_list *bl,va_list ap)
{
	s_mob_data *md = (s_mob_data*)bl;
 ...
 *------------------------------------------*/
static int mob_ai_sub_lazy(struct s_mob_data *md, va_list args)
{
	unsigned int tick;
	...
	if( DIFF_TICK(md->next_walktime,tick) < 0 && status_has_mode(&md->status,e_MonsterMode::CANMOVE) && unit_can_move(&md->bl) )
	{
 ...
 * @author [Cydh]
 **/
void mob_setdropitem_option(s_item *itm, struct s_mob_drop *mobdrop) {
 ...
 *------------------------------------------*/
static s_item_drop* mob_setdropitem(struct s_mob_drop *mobdrop, int qty, unsigned short mob_id)
{
	struct s_item_drop *drop = ers_alloc(item_drop_ers, struct s_item_drop);
	memset(&drop->item_data, 0, sizeof(struct s_item));
 ...
 *------------------------------------------*/
static struct s_item_drop* mob_setlootitem(struct s_mob_lootitem *item, unsigned short mob_id)
{
	struct s_item_drop *drop = ers_alloc(item_drop_ers, struct s_item_drop);
	memcpy(&drop->item_data, item, sizeof(struct s_item));
...
{
	struct s_item_drop_list *list;
	struct s_item_drop *ditem;

	list = (struct s_item_drop_list *)data;
	ditem = list->item;

	while (ditem) {
		struct s_item_drop *ditem_prev;
 ...
 *------------------------------------------*/
static void mob_item_drop(struct s_mob_data *md, struct s_item_drop_list *dlist, struct s_item_drop *ditem, int loot, int drop_rate, unsigned short flag)
{
	TBL_PC* sd;
	bool test_autoloot;
	...
	if( test_autoloot ) {	//Autoloot.
		struct s_party_data *p = party_search(sd->status.party_id);
 ...
 *
 *------------------------------------------*/
int mob_deleteslave_sub(struct s_block_list *bl,va_list ap)
{
	struct s_mob_data *md;
	int id;

	nullpo_ret(bl);
	nullpo_ret(md = (struct s_mob_data *)bl);
...
void mob_log_damage(struct s_mob_data *md, struct s_block_list *src, int damage)
{
	...
	switch( src->type )
	{
		...
		case BL_HOM:
		{
			struct s_homun_data *hd = (TBL_HOM*)src;
			flag = MDLF_HOMUN;
			if( hd->master )
		...
		case BL_MER:
		{
			struct s_mercenary_data *mer = (TBL_MER*)src;
			if( mer->master )
		...
		case BL_ELEM:
		{
			struct s_elemental_data *ele = (TBL_ELEM*)src;
			if( ele->master )
...
int mob_dead(struct s_mob_data *md, struct s_block_list *src, int type)
{
	...
	if(src && src->type == BL_MOB)
		mob_unlocktarget((struct s_mob_data *)src,tick);
	...
	{ // Item Drop
		struct s_item_drop_list *dlist = ers_alloc(item_drop_list_ers, struct s_item_drop_list);
		struct s_item_drop *ditem;
		struct s_item_data* it = NULL;
		int drop_rate;
#ifdef RENEWAL_DROP
	...
	if( !rebirth ) { // Only trigger event on final kill
		...
		if (sd) {
			s_mob_db *mission_mdb = mobdb_exists(sd->mission_mobid);
...
int mob_guardian_guildchange(struct s_mob_data *md)
{
	struct s_guild *g;
	nullpo_ret(md);
...
int mob_class_change (struct s_mob_data *md, int mob_id)
{
	...
	if (md->lootitems == NULL && status_has_mode(&md->db->status,e_MonsterMode::LOOTER))
		md->lootitems = (struct s_mob_lootitem *)aCalloc(LOOTITEM_SIZE,sizeof(struct s_mob_lootitem));
 ...
 * Added by RoVeRT
 *------------------------------------------*/
int mob_warpslave_sub(struct s_block_list *bl,va_list ap)
{
	struct s_mob_data *md=(struct s_mob_data *)bl;
	struct s_block_list *master;
	short x,y,range=0;
	master = va_arg(ap, struct s_block_list*);
	range = va_arg(ap, int);
 ...
 *------------------------------------------*/
int mob_countslave_sub(struct s_block_list *bl,va_list ap)
{
	int id;
	struct s_mob_data *md;
	id=va_arg(ap,int);
...
int mob_skill_id2skill_idx(int mob_id,uint16 skill_id)
{
	...
	struct s_mob_skill *ms=mob_db(mob_id)->skill;
 ...
 *------------------------------------------*/
int mob_getfriendhprate_sub(struct s_block_list *bl,va_list ap)
{
	int min_rate, max_rate,rate;
	struct s_block_list **fr;
	struct s_mob_data *md;

	md = va_arg(ap,struct s_mob_data *);
	min_rate=va_arg(ap,int);
	max_rate=va_arg(ap,int);
	fr=va_arg(ap,struct s_block_list **);
...
static struct s_block_list *mob_getfriendhprate(struct s_mob_data *md,int min_rate,int max_rate)
{
	struct s_block_list *fr=NULL;
	int type = BL_MOB;
 ...
 *------------------------------------------*/
struct s_block_list *mob_getmasterhpltmaxrate(struct s_mob_data *md,int rate)
{
	if( md && md->master_id > 0 )
	{
		struct s_block_list *bl = map_id2bl(md->master_id);
 ...
 *------------------------------------------*/
int mob_getfriendstatus_sub(struct s_block_list *bl,va_list ap)
{
	int cond1,cond2;
	struct s_mob_data **fr, *md, *mmd;
	int flag=0;

	nullpo_ret(bl);
	nullpo_ret(md=(struct s_mob_data *)bl);
	nullpo_ret(mmd=va_arg(ap,struct s_mob_data *));
	...
	cond1=va_arg(ap,int);
	cond2=va_arg(ap,int);
	fr=va_arg(ap,struct s_mob_data **);
	if( cond2==-1 ){
		int j;
...
struct s_mob_data *mob_getfriendstatus(struct s_mob_data *md,int cond1,int cond2)
{
	struct s_mob_data* fr = NULL;
	nullpo_ret(md);
...
{
	struct s_mob_skill *ms;
	struct s_block_list *fbl = NULL; //Friend bl, which can either be a BL_PC or BL_MOB depending on the situation. [Skotlex]
	struct s_block_list *bl;
	struct s_mob_data *fmd = NULL;
	int i,j,n;
	short skill_target;
	...
	if (!battle_config.mob_skill_rate || md->ud.skilltimer != INVALID_TIMER || !md->db->maxskill || status_has_mode(&md->status,e_MonsterMode::NOCAST_SKILL))
		return 0;
	...
	for (n = 0; n < md->db->maxskill; i++, n++) {
		...
		//Execute skill
		skill_target = status_has_mode(&md->db->status,e_MonsterMode::RANDOMTARGET) ? MST_RANDOM : ms[i].target;
		...
		if ( ms[ i ].msg_id ){ //Display color message [SnakeDrak]
			struct s_mob_chat *mc = mob_chat(ms[i].msg_id);
...
int mob_clone_spawn(s_map_session_data *sd, int16 m, int16 x, int16 y, const char *event, int master_id, e_MonsterMode mode, int flag, unsigned int duration)
{
	...
	db = mob_db_data[mob_id]=(struct s_mob_db*)aCalloc(1, sizeof(struct s_mob_db));
	status = &db->status;
	strcpy(db->sprite,sd->status.name);
	strcpy(db->name,sd->status.name);
	strcpy(db->jname,sd->status.name);
	db->lv=status_get_lv(&sd->bl);
	memcpy(status, &sd->base_status, sizeof(struct s_status_data));
	...
	if (static_cast<int>(mode) != 0) //User provided mode.
		status->mode = (e_MonsterMode)mode;
	else if (flag&1) //Friendly Character, remove looting.
		status->mode = (e_MonsterMode)(status->mode & ~e_MonsterMode::LOOTER);
	status->hp = status->max_hp;
	status->sp = status->max_sp;
	memcpy(&db->vd, &sd->vd, sizeof(struct s_view_data));
	db->base_exp=1;
	db->job_exp=1;
	...
	for (i=0,j = MAX_SKILL_TREE-1;j>=0 && i< MAX_MOBSKILL ;j--) {
		...
		memset (&ms[i], 0, sizeof(struct s_mob_skill));
		...
		if (inf&INF_ATTACK_SKILL) {
			...
			if (i+1 < MAX_MOBSKILL) { //duplicate this so it also triggers on self.
				memcpy(&ms[i+1], &ms[i], sizeof(struct s_mob_skill));
				db->maxskill = ++i;
				ms[i].target = MST_SELF;
				ms[i].cond1 = MSC_MYHPLTMAXRATE;
			}
...
static int mob_makedummymobdb(int mob_id)
{
	...
	mob_dummy = (struct s_mob_db*)aCalloc(1, sizeof(struct s_mob_db)); //Initializing the dummy mob.
...
{
	struct s_mob_db *db, entry;
	struct s_status_data *status;
	int mob_id, i;
	double exp, maxhp;
	struct s_mob_data data;
	...
	status->mode = (e_MonsterMode)strtol(str[25], NULL, 0);
	if (!battle_config.monster_active_enable)
		status->mode = (e_MonsterMode)(status->mode & ~e_MonsterMode::AGGRESSIVE);

	if (status_has_mode(status,e_MonsterMode::STATUS_IMMUNE|e_MonsterMode::KNOCKBACK_IMMUNE|e_MonsterMode::DETECTOR))
	...
	data.bl.type = BL_MOB;
	data.level = db->lv;
	memcpy(&data.status, status, sizeof(struct s_status_data));
	...
	if (mob_db_data[mob_id] == NULL)
		mob_db_data[mob_id] = (struct s_mob_db*)aCalloc(1, sizeof(struct s_mob_db));
	else
		//Copy over spawn data
		memcpy(&db->spawn, mob_db_data[mob_id]->spawn, sizeof(db->spawn));

	memcpy(mob_db_data[mob_id], db, sizeof(struct s_mob_db));
	return true;
}
...
static bool mob_readdb_mobavail(char* str[], int columns, int current)
{
	...
	memset(&mob_db_data[mob_id]->vd, 0, sizeof(struct s_view_data));
...
static bool mob_parse_row_chatdb(char* fields[], int columns, int current)
{
	...
	if (mob_chat_db[msg_id] == NULL)
		mob_chat_db[msg_id] = (struct s_mob_chat*)aCalloc(1, sizeof (struct s_mob_chat));
...
static bool mob_parse_row_mobskilldb(char** str, int columns, int current)
{
	...
	struct s_internal_mob_skill *skill = NULL;
	struct s_mob_skill *ms = NULL;
	int mob_id;
	int i = 0, j, tmp;
	...
	if (!(skill = (struct s_internal_mob_skill *)idb_get(mob_skill_db, mob_id)))
		CREATE(skill, struct s_internal_mob_skill, 1);
	...
	{	//Adds a mode to the mob.
		...
		if (!(ms->val[1]&e_MonsterMode::AGGRESSIVE))
			ms->val[3] |= static_cast<int>(e_MonsterMode::AGGRESSIVE);
 ...
 **/
static int mob_item_drop_ratio_free(u_DBKey key, s_DBData *data, va_list ap) {
	...
	for( i = 0; i <= MAX_MOB_DB; i++ ){
		...
		for( j = 0; j < MAX_MOB_DROP_TOTAL; j++ ){
			...
			if (mob->race2 == RC2_TREASURE) {
				...
				bool is_mvp = status_has_mode(&mob->status,e_MonsterMode::MVP);
 ...
 **/
static void mob_skill_db_set_single_sub(struct s_mob_db *mob, struct s_internal_mob_skill *skill) {
	uint8 i;
 ...
 * @param skill
 **/
static void mob_skill_db_set_single(struct s_internal_mob_skill *skill) {
	...
	// Global skill
		...
		for (i = 0; i < MAX_MOB_DB; i++) {
			...
			if (mob == mob_dummy)
				continue;
			if (   (!(id&1) && status_has_mode(&mob->status,e_MonsterMode::STATUS_IMMUNE)) // Bosses
				|| (!(id&2) && !status_has_mode(&mob->status,e_MonsterMode::STATUS_IMMUNE)) // Normal monsters
				)
				continue;
			mob_skill_db_set_single_sub(mob, skill);
		}
 ...
 **/
static int mob_skill_db_free(u_DBKey key, s_DBData *data, va_list ap) {
 ...
 **/
static int mob_summon_db_free(u_DBKey key, s_DBData *data, va_list ap) {
...
static void mob_skill_db_set(void) {
	s_DBIterator *iter = db_iterator(mob_skill_db);
	struct s_internal_mob_skill *skill = NULL;

	for (skill = (struct s_internal_mob_skill *)dbi_first(iter);  dbi_exists(iter); skill = (struct s_internal_mob_skill *)dbi_next(iter)) {
...
void mob_db_load(bool is_reload){
	...
	mob_db_data[0] = (s_mob_db*)aCalloc(1, sizeof (s_mob_db));	//This mob is used for random spawns
	mob_makedummymobdb(0); //The first time this is invoked, it creates the dummy mob
	if( !is_reload ) {
		// on mobdbreload it's not neccessary to execute this
		// item ers needs to be allocated only once
		item_drop_ers = ers_new(sizeof(s_item_drop),"mob.cpp::item_drop_ers",ERS_OPT_CLEAN);
		item_drop_list_ers = ers_new(sizeof(s_item_drop_list),"mob.cpp::item_drop_list_ers",ERS_OPT_NONE);
	}
 ...
 * @return 0
 */
static int mob_reload_sub( s_mob_data *md, va_list args ){
 ...
 * @return 0
 */
static int mob_reload_sub_npc( s_npc_data *nd, va_list args ){
