...
struct s_battleground_data* bg_team_search(int bg_id) // Search a BG Team using bg_id
{
	if (!bg_id)
		return NULL;
...
int bg_team_join(int bg_id, s_map_session_data* sd) // Player joins team
{
	int                         i;
	struct s_battleground_data* bg = bg_team_search(bg_id);
...
int bg_team_leave(s_map_session_data* sd, int flag) // Single Player leaves team
{
	int                         i, bg_id;
...
int bg_member_respawn(s_map_session_data* sd) // Respawn after killed
{
	struct s_battleground_data* bg;
