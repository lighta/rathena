...
struct s_homun_skill_tree_entry hskill_tree[MAX_HOMUNCULUS_CLASS][MAX_HOM_SKILL_TREE];
...
static struct s_view_data hom_viewdb[MAX_HOMUNCULUS_CLASS];
 ...
 * @return vd
 */
struct s_view_data* hom_get_viewdata(int class_) //Returns the viewdata for homunculus
{
	if (homdb_checkid(class_))
...
int hom_levelup(struct s_homun_data* hd)
{
	...
	struct s_h_stats*    min = NULL, * max = NULL;
 ...
 */
static bool hom_change_class(struct s_homun_data* hd, short class_)
{
	int i;
...
void hom_alloc(s_map_session_data* sd, struct s_homunculus* hom)
{
	...
	sd->hd      = hd = (struct s_homun_data*)aCalloc(1, sizeof(struct s_homun_data));
	hd->bl.type = BL_HOM;
 ...
 * @param hd
 */
void hom_reset_stats(struct s_homun_data* hd) //Resets a homunc stats back to zero (but doesn't touches hunger or intimacy)
{
...
int hom_shuffle(struct s_homun_data* hd)
{
	...
	if (hd->homunculus.class_ == hd->homunculusDB->evo_class) {
		...
		struct s_h_stats*    max = &hd->homunculusDB->emax, * min = &hd->homunculusDB->emin;
		hom->max_hp += rnd_value(min->HP, max->HP);
		hom->max_sp += rnd_value(min->SP, max->SP);
		hom->str    += 10 * rnd_value(min->str, max->str);
		hom->agi    += 10 * rnd_value(min->agi, max->agi);
