...
extern struct s_Script_Config script_config;
...
struct s_reg_db {
	struct s_DBMap *vars;
	struct s_DBMap *arrays;
};

struct s_script_retinfo {
	struct s_reg_db scope;        ///< scope variables
	struct s_script_code* script; ///< script code
	int pos;                    ///< script location
	int nargs;                  ///< argument count
	int defsp;                  ///< default stack pointer
};

struct s_script_data {
	enum e_c_op type;
...
struct s_script_code {
	...
	struct s_reg_db local;
	unsigned short instances;
};

struct s_script_stack {
	int sp;                         ///< number of entries in the stack
	int sp_max;                     ///< capacity of the stack
	int defsp;
	struct s_script_data *stack_data; ///< stack
	struct s_reg_db scope;            ///< scope variables
};
...
struct s_script_regstr {
	int64 index;
	char* data;
};

struct s_script_array {
...
enum e_random_option_attribute {
	ROA_ID = 0,
	ROA_VALUE,
	ROA_PARAM,
};
 ...
 **/
extern struct s_eri *array_ers;
extern s_DBMap *st_db;
extern unsigned int active_scripts;
extern unsigned int next_id;
extern struct s_eri *st_ers;
extern struct s_eri *stack_ers;
...
void run_script_sub(struct s_script_code *rootscript,int pos,int rid,int oid, char* file, int lineno);
void run_script(struct s_script_code *rootscript,int pos,int rid,int oid);

int set_reg(struct s_script_state* st, struct s_map_session_data* sd, int64 num, const char* name, const void* value, struct s_reg_db *ref);
...
struct s_linkdb_node* script_erase_sleepdb(struct s_linkdb_node *n);
...
void setd_sub(struct s_script_state *st, struct s_map_session_data *sd, const char *varname, int elem, void *value, struct s_reg_db *ref);
