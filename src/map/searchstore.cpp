...
typedef bool (*searchstore_search_t)(struct s_map_session_data* sd, unsigned short nameid);
typedef bool (*searchstore_searchall_t)(struct s_map_session_data* sd, const struct s_search_store_search* s);
 ...
 * @return : store type
 */
static bool searchstore_hasstore(struct s_map_session_data* sd, unsigned char type)
{
	switch( type ) {
 ...
 * @return : store ID
 */
static int searchstore_getstoreid(struct s_map_session_data* sd, unsigned char type)
{
	switch( type ) {
...
void searchstore_query(struct s_map_session_data* sd, unsigned char type, unsigned int min_price, unsigned int max_price, const unsigned short* itemlist, unsigned int item_count, const unsigned short* cardlist, unsigned int card_count)
{
	...
	for( pl_sd = (struct s_map_session_data*)dbi_first(iter); dbi_exists(iter);  pl_sd = (struct s_map_session_data*)dbi_next(iter) ) {
