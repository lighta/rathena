 ...
 */
static int inventory_tosql(uint32 char_id, s_storage* p)
{
 ...
 * @return 0 if success, or error count
 */
static int storage_tosql(uint32 account_id, s_storage* p)
{
 ...
 * @return 0 if success, or error count
 */
static int cart_tosql(uint32 char_id, s_storage* p)
{
 ...
 * @return 0 if success, or error count
 */
bool guild_storage_tosql(int guild_id, s_storage* p)
{
...
static void mapif_storage_data_loaded(int fd, uint32 account_id, char type, s_storage entries, bool result) {
	...
	WFIFOB(fd, 9) = result;
	memcpy(WFIFOP(fd, 10), &entries, sizeof(s_storage));
	WFIFOSET(fd, size);
}
