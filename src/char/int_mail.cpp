...
static bool mail_loadmessage(int mail_id, s_mail_message* msg);
...
int mail_fromsql(uint32 char_id, s_mail_data* md)
{
	int i;
	char *data;

	memset(md, 0, sizeof(s_mail_data));
	md->amount = 0;
	md->full = false;
...
static bool mail_loadmessage(int mail_id, s_mail_message* msg)
{
	...
	||  SQL_SUCCESS != Sql_NextRow(sql_handle) ){
		...
		Sql_GetData(sql_handle,10, &data, NULL); msg->type = (e_mail_inbox_type)atoi(data);
	...
	memset(msg->item, 0, sizeof(s_item) * MAIL_MAX_ITEM);
...
static int mail_timer_sub( int limit, e_mail_inbox_type type ){
	struct{
		int mail_id;
		int char_id;
		int account_id;
	...
	for( i = 0; i < MAIL_MAX_INBOX; i++ ){
		...
		if( ( character = (s_online_char_data*)idb_get(char_get_onlinedb(), mails[i].account_id) ) != NULL && character->server >= 0 ){
 ...
 *------------------------------------------*/
static void mapif_Mail_sendinbox(int fd, uint32 char_id, unsigned char flag, e_mail_inbox_type type)
{
	s_mail_data md;
...
{
	mapif_Mail_sendinbox(fd, RFIFOL(fd,2), RFIFOB(fd,6), (e_mail_inbox_type)RFIFOB(fd,7));
}
...
static void mapif_Mail_getattach(int fd, uint32 char_id, int mail_id, int type)
{
	...
	WFIFOHEAD(fd, sizeof(s_item)*MAIL_MAX_ITEM + 16);
	WFIFOW(fd,0) = 0x384a;
	WFIFOW(fd,2) = sizeof(s_item)*MAIL_MAX_ITEM + 16;
	WFIFOL(fd,4) = char_id;
	WFIFOL(fd,8) = mail_id;
	if( type & MAIL_ATT_ZENY ){
	...
	if( type & MAIL_ATT_ITEM ){
		memcpy(WFIFOP(fd, 16), &msg.item, sizeof(s_item)*MAIL_MAX_ITEM);
	}else{
		memset(WFIFOP(fd, 16), 0, sizeof(s_item)*MAIL_MAX_ITEM);
	}
	WFIFOSET(fd,WFIFOW(fd,2));
}
 ...
 * Send Mail
 *------------------------------------------*/
static void mapif_Mail_send(int fd, s_mail_message* msg)
{
	int len = sizeof(s_mail_message) + 4;

	WFIFOHEAD(fd,len);
	WFIFOW(fd,0) = 0x384d;
	WFIFOW(fd,2) = len;
	memcpy(WFIFOP(fd,4), msg, sizeof(s_mail_message));
	WFIFOSET(fd,len);
}
...
static void mapif_parse_Mail_send(int fd)
{
	...
	if(RFIFOW(fd,2) != 8 + sizeof(s_mail_message))
		return;

	memcpy(&msg, RFIFOP(fd,8), sizeof(s_mail_message));
...
{
	s_mail_message msg;
	memset(&msg, 0, sizeof(s_mail_message));
	...
	if( item != NULL ){
		...
		for( i = 0; i < amount && i < MAIL_MAX_ITEM; i++ ){
			memcpy(&msg.item[i], &item[i], sizeof(s_item));
		}
	}
