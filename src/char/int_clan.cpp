...
s_clan* inter_clan_fromsql(int clan_id){
	s_clan* clan;
	char* data;
	size_t len;
	int i;
	...
	clan = (s_clan*)idb_get(clan_db, clan_id);
	...
	CREATE(clan, s_clan, 1);
	memset(clan, 0, sizeof(s_clan));
	...
	for( i = 0; i < MAX_CLANALLIANCE && SQL_SUCCESS == Sql_NextRow(sql_handle); i++ ){
		s_clan_alliance* a = &clan->alliance[i];
...
int mapif_clan_info( int fd ){
	...
	length = 4 + db_size(clan_db) * sizeof( s_clan );
	...
	for( clan = (s_clan*)dbi_first(iter); dbi_exists(iter); clan = (s_clan*)dbi_next(iter) ){
		memcpy( WFIFOP( fd, offset ), clan, sizeof( s_clan ) );
		offset += sizeof( s_clan );
	}
	dbi_destroy(iter);
...
static void mapif_clan_refresh_onlinecount( int fd, s_clan* clan ){
	unsigned char buf[8];
...
static void mapif_parse_clan_member_left( int fd ){
	s_clan* clan = (s_clan*)idb_get(clan_db, RFIFOL(fd,2) );
