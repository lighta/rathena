...
#include "../common/c_rAString.h" // is_email_valid
#include "../common/c_timer.h" //timestamp2string
...
namespace ra {
  namespace login {
     ...
     */
    int c_ModuleChrif::logchrif_parse(int fd) {
      ...
      while (RFIFOREST(fd) >= 2) {
        ...
        switch (command) {
          case 0x2712: next = logchrif_parse_reqauth(fd, cid, ip);
            break;
          case 0x2714: next = logchrif_parse_ackusercount(fd, cid);
            break;
          case 0x2716: next = logchrif_parse_reqaccdata(fd, cid, ip);
            break;
          case 0x2719: next = logchrif_parse_keepalive(fd);
            break;
          case 0x2720: next = logchrif_parse_accinfo(fd);
            break; //@accinfo from inter-server
          case 0x2722: next = logchrif_parse_reqchangemail(fd, cid, ip);
            break;
          case 0x2724: next = logchrif_parse_requpdaccstate(fd, cid, ip);
            break;
          case 0x2725: next = logchrif_parse_reqbanacc(fd, cid, ip);
            break;
          case 0x2727: next = logchrif_parse_reqchgsex(fd, cid, ip);
            break;
          case 0x2728: next = logchrif_parse_upd_global_accreg(fd, cid, ip);
            break;
          case 0x272a: next = logchrif_parse_requnbanacc(fd, cid, ip);
            break;
          case 0x272b: next = logchrif_parse_setacconline(fd, cid);
            break;
          case 0x272c: next = logchrif_parse_setaccoffline(fd);
            break;
          case 0x272d: next = logchrif_parse_updonlinedb(fd, cid);
            break;
          case 0x272e: next = logchrif_parse_req_global_accreg(fd);
            break;
          case 0x2736: next = logchrif_parse_updcharip(fd, cid);
            break;
          case 0x2737: next = logchrif_parse_setalloffline(fd, cid);
            break;
#if PACKETVER_SUPPORTS_PINCODE
          case 0x2738: next = logchrif_parse_updpincode(fd);
            break;
          case 0x2739: next = logchrif_parse_pincode_authfail(fd);
            break;
#endif
          case 0x2742: next = logchrif_parse_reqvipdata(fd);
            break; //Vip sys
          default:
     ...
     * @return 0
     */
    int c_ModuleChrif::logchrif_sync_ip_addresses(int tid, unsigned int tick, int id, intptr_t data) {
      uint8 buf[2];
      ShowInfo("IP Sync in progress...\n");
      WBUFW(buf, 0) = 0x2735;
      c_ModuleChrif::smGetInstance().logchrif_sendallwos(-1, buf, 2);
      return 0;
    }
     ...
     * @return 0 not enough info transmitted, 1 success
     */
    int c_ModuleChrif::logchrif_parse_ackusercount(int fd, int id) {
      if (RFIFOREST(fd) < 6)
        return 0;
      else {
     ...
     */
    int c_ModuleChrif::logchrif_send_accdata(int fd, uint32 aid) {
      ...
      safestrncpy(WFIFOCP(fd, 52), birthdate.c_str(), birthdate.size());
      safestrncpy(WFIFOCP(fd, 63), pincode.c_str(), pincode.size());
     ...
     * @param mapfd
     */
    int c_ModuleChrif::logchrif_sendvipdata(int fd, struct s_mmo_account acc, unsigned char flag, int mapfd) {
#ifdef VIP_ENABLE
      WFIFOHEAD(fd, 19);
      WFIFOW(fd, 0) = 0x2743;
     ...
     * @return 1 success
     */
    int c_ModuleChrif::logchrif_parse_keepalive(int fd) {
      RFIFOSKIP(fd, 2);
      WFIFOHEAD(fd, 2);
      WFIFOW(fd, 0) = 0x2718;
      WFIFOSET(fd, 2);
    ...
    int c_ModuleChrif::logchrif_parse_reqchangemail(int fd, int id, char* ip) {
      if (RFIFOREST(fd) < 86)
        ...
        uint32 account_id = RFIFOL(fd, 2);
        actual_email = std::string(RFIFOCP(fd, 6), 40);
        new_email = std::string(RFIFOCP(fd, 46), 40);
        RFIFOSKIP(fd, 86);

        if (c_rAString::is_valid_email(actual_email) == false)
        ...
        else if (c_rAString::is_valid_email(new_email) == false)
        ...
        else if (strcmpi(new_email.c_str(), "a@a.com") == 0)
     ...
     * @return 0 not enough info transmitted, 1 success
     */
    int c_ModuleChrif::logchrif_parse_setacconline(int fd, int id) {
      if (RFIFOREST(fd) < 6)
        return 0;
     ...
     * @return 0 not enough info transmitted, 1 success
     */
    int c_ModuleChrif::logchrif_parse_setaccoffline(int fd) {
      if (RFIFOREST(fd) < 6)
        return 0;
     ...
     * @return 0 not enough info transmitted, 1 success
     */
    int c_ModuleChrif::logchrif_parse_updonlinedb(int fd, int id) {
      if (RFIFOREST(fd) < 4 || RFIFOREST(fd) < RFIFOW(fd, 2))
        return 0;
      else {
        uint32 i, users;
     ...
     * @return 0 not enough info transmitted, 1 success
     */
    int c_ModuleChrif::logchrif_parse_req_global_accreg(int fd) {
      if (RFIFOREST(fd) < 10)
        return 0;
      else {
        s_AccountDB* accounts = login_get_accounts_db();
     ...
     * @return 0 not enough info transmitted, 1 success
     */
    int c_ModuleChrif::logchrif_parse_updcharip(int fd, int id) {
      if (RFIFOREST(fd) < 6)
        return 0;
     ...
     * @return 1 success
     */
    int c_ModuleChrif::logchrif_parse_setalloffline(int fd, int id) {
     ...
     */
    int c_ModuleChrif::logchrif_parse_updpincode(int fd) {
     ...
     */
    int c_ModuleChrif::logchrif_parse_pincode_authfail(int fd) {
      if (RFIFOREST(fd) < 6)
        return 0;
      else {
        struct s_mmo_account acc;
        ...
        if (accounts->load_num(accounts, &acc, RFIFOL(fd, 2))) {
          ...
          login_log(host2ip(acc.last_ip.c_str()), acc.userid.c_str(), 100, "PIN Code check failed");
        }
     ...
     */
    int c_ModuleChrif::logchrif_parse_reqvipdata(int fd) {
#ifdef VIP_ENABLE
      if (RFIFOREST(fd) < 15)
        return 0;
     ...
     */
    int c_ModuleChrif::logchrif_parse_accinfo(int fd) {
      if (RFIFOREST(fd) < 23)
        return 0;
      else {
        ...
        if (accounts->load_num(accounts, &acc, account_id)) {
          ...
          safestrncpy(WFIFOCP(fd, 71), acc.last_ip.c_str(), acc.last_ip.size());
          safestrncpy(WFIFOCP(fd, 87), acc.lastlogin.c_str(), acc.lastlogin.size());
          safestrncpy(WFIFOCP(fd, 111), acc.birthdate.c_str(), acc.birthdate.size());
          if ((unsigned int) u_group >= acc.group_id) {
            safestrncpy(WFIFOCP(fd, 122), acc.pass.c_str(), acc.pass.size());
            safestrncpy(WFIFOCP(fd, 155), acc.pincode.c_str(), acc.pincode.size());
          } else {
            memset(WFIFOP(fd, 122), '\0', 33);
            memset(WFIFOP(fd, 155), '\0', PINCODE_LENGTH);
          }
          safestrncpy(WFIFOCP(fd, 155 + PINCODE_LENGTH), acc.userid.c_str(), acc.userid.size());
          WFIFOSET(fd, len);
        } else {
     ...
     */
    void c_ModuleChrif::logchrif_server_init(int id) {
     ...
     * @param id: id of char-serv (should be >0, FIXME)
     */
    void c_ModuleChrif::logchrif_server_destroy(int id) {
     ...
     * @param id: id of char-serv (should be >0, FIXME)
     */
    void c_ModuleChrif::logchrif_server_reset(int id) {
     ...
     * @param id: id of char-serv (should be >0, FIXME)
     */
    void c_ModuleChrif::logchrif_on_disconnect(int id) {
     ...
     */
    void c_ModuleChrif::do_init_loginchrif(void) {
      int i;
     ...
     */
    void c_ModuleChrif::do_shutdown_loginchrif(void) {
      int id;
     ...
     */
    void c_ModuleChrif::do_final_loginchrif(void) {
      int i;
      for (i = 0; i < ARRAYLENGTH(ch_server); ++i)
