    ...
    int logclif_parse(int fd) {
      ...
      while (RFIFOREST(fd) >= 2) {
        ...
        switch (command) {
          ...
          case 0x0200: next = logclif_parse_keepalive(fd);
            break;
            // client md5 hash (binary)
          case 0x0204: next = logclif_parse_updclhash(fd, sd);
            break;
          ...
          case 0x01db: next = logclif_parse_reqkey(fd, sd);
            break;
            // Connection request of a char-server
          case 0x2710: logclif_parse_reqcharconnec(fd, sd, ip);
